...
### 方法一：手动安装
1. 将 `peak_valley_energy` 文件夹复制到你的 Home Assistant 配置目录：
   ```
...
### 方法二：Docker 容器内已安装
如果你的集成已放在 `/root/docker/hass/custom_components/`，重启容器即可：
```bash
docker restart hass
```
...
### 服务（未来扩展）
计划支持的服务：
- `peak_valley_energy.reset_data` - 手动重置统计数据
- `peak_valley_energy.recalculate` - 重新计算历史数据
